...
generateAndSave :: R.Connection
                -> BC.ByteString
                -> ActionM ()
generateAndSave conn uri = do
  shawty <- liftIO shortyGen
  let shorty = BC.pack shawty
  duplicateLookup <- liftIO (getURI conn uri)
  case duplicateLookup of
    Left reply ->
        text (TL.pack (show reply))
    Right mbBS -> case mbBS of
      Nothing -> do
        resp <-
          liftIO (saveURI conn shorty uri)
        html (shortyCreated resp shawty)
      Just _ -> generateAndSave conn uri
